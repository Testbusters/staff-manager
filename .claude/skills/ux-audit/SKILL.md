---
name: ux-audit
description: Analyse user experience quality of staff-manager flows. Simulates real user journeys per role using Playwright, evaluates 7 dimensions (task completion, consistency, feedback, navigation, cognitive load, error recovery, user confidence) anchored to ISO 9241-11, Nielsen's 10 heuristics, and Baymard form research. Includes heuristic sweep (H3/H5/H6/H8/H9), Baymard form checklist, error message quality rating, and wasted-click detection. Produces a severity-ranked report (Critical / Major / Minor). Requires npm run dev on localhost:3000.
user-invocable: true
model: opus
context: fork
...
- `target:page:/rimborsi/nuova` → only flows that touch this route
...
| **D2 — Interaction consistency** | Effectiveness | H4 Consistency & standards | Do similar operations work the same way across sections? | CRUD patterns across compensi / rimborsi / ticket / contenuti / corsi |
...
**Steps**:
1. Navigate to `/corsi`
2. Observe: are the 3 sections (Corsi disponibili, Candidature in attesa, Docenze) clearly separated?
3. Click on a corso → `/corsi/[id]`
...
**Steps**:
1. Login → `/`
2. Navigate to `/corsi/assegnazione`
3. Verify: can see list of lezioni with assignable collaboratori
...
#### F8 — Responsabile: caricamento compensazioni da GSheet
**Role**: resp
**Steps**: `/approvazioni` → Carica → GSheet URL input → preview → confirm
**Evaluate**: D1 (import path discoverable?), D3 (feedback on each step + BF1?), D6 (what if GSheet URL is wrong?), D7 (confirmation before bulk import executes?)

#### F9 — Collaboratore: firma documento
**Role**: collab
...
#### F12 — Responsabile cittadino: valutazioni corsi
**Role**: resp_citt
**Prerequisite**: same as F7 — skip if no account.
**Read before simulating**: look up `/corsi/valutazioni` in `docs/sitemap.md` 'Componenti chiave' column → read the listed files
**Steps**:
1. Navigate to `/corsi/valutazioni`
2. Find a lezione with a completed docenza
3. Enter a valutazione for a collaboratore
...
- F1 (rimborso) vs F3 (ticket): do both new-record forms have the same pattern? (CTA placement, field order, submit label, post-submit behavior, cancel path)
- F2 (comunicazioni detail) vs any other detail page: do detail pages have equivalent back navigation?
- Any candidatura/application flow vs F3 (ticket new): is the "create" flow pattern consistent (same toast, same redirect)?
...
|---|---|---|---|
| H3 | User control & freedom | Every dialog and multi-step form has a functional Cancel/Annulla button. Verify dismiss returns to correct state with no unintended side effects (no draft saved, no partial commit). | Any dialog without a clear exit, or cancel causes data loss |
...
|---|---|---|---|
| [e.g. Post-submit redirect] | /rimborsi/nuova vs /ticket/nuova | [difference] | Minor |
