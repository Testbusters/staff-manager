---
name: ui-audit
description: Full UI quality audit for staff-manager. Covers design system token compliance, accessibility (WCAG 2.2 static + axe-core live scan), Tailwind v4 breaking-change detection, keyboard/interaction patterns, form label completeness, empty states, table rules, loading.tsx coverage. Uses docs/sitemap.md as authoritative file inventory. Static mode runs concurrently with Playwright skills; full mode (dev server up) adds axe-core browser scan.
user-invocable: true
model: sonnet
context: fork
...
You are performing a comprehensive UI quality audit of the staff-manager Next.js app.
...
**B — Component groups by layout type** (derived from the "Componenti chiave" column — do not hardcode):
- `full-list` pages: routes with DataTable or list-style Componenti chiave
- `detail` pages: routes with `/[id]` in the path
- `form / wizard` pages: routes with Form, wizard, or onboarding components
- `tabs` pages: routes with Tabs in Componenti chiave
- `dashboard`: the root `app/(app)/page.tsx`
- `feed`: content routes (comunicazioni, eventi, opportunita)
- `calendar / grid` pages: routes with CorsiCalendario or similar grid components in Componenti chiave
...
Pattern: lines containing `<Input|<Textarea|<Select` (shadcn components) that do NOT also contain `aria-label` or `aria-labelledby` or `id=` (indicating a Label htmlFor association is possible).
...
Run: `find app -name "loading.tsx" | sort` and cross-reference with the sitemap.
...
Then check the first 3 Componenti chiave components from the sitemap for the target scope: if any page-level component (`page.tsx` or its direct named child) carries `'use client'` AND contains no hooks or browser APIs, flag it — the client boundary is unnecessarily high.
