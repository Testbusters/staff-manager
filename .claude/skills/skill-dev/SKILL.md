...
Read in order:
1. `docs/sitemap.md` — build target file list (page files + key components per route + lib files referenced). If not in the current worktree, read from `/Users/MarcoG/Projects/staff-manager/docs/sitemap.md`.
...
From sitemap.md "Componenti chiave" column, identify the 3 components with the most listed sub-responsibilities. Read each candidate file.
...
For each wizard/multi-step form (OnboardingWizard, rimborsi stepper, document upload wizard): read the top-level component. Count props passed down 3+ levels without being consumed at intermediate levels. Flag if prop count > 5 at depth > 2.
Check for "data clumps" — groups of 3+ related variables always passed as separate props (e.g. `userId`, `collaboratorId`, `communityId` should be a single object type).
...
If a `page.tsx` or `layout.tsx` has `'use client'` but only one subcomponent uses client features, split the interactive part into a named Client Component. Marking the whole page forces ALL imports into the client bundle.
...
Grep: `export async function` in `.ts`/`.tsx` files outside `app/api/`. For each match, verify the file has `'use server'` at the top OR the function is a React Server Component. A Server Action missing `'use server'` silently executes client-side and may expose server-only logic or fail on server resources.
