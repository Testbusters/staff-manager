...
Scope: staff-manager — Next.js 16, TypeScript, Supabase, Vitest, Playwright, shadcn/ui, Vercel.
...
- **TypeScript check before every commit**: `npx tsc --noEmit` must pass. Type errors are blocking — they predict runtime failures in a TypeScript codebase. *(Source: 7)*
- **Build pass required**: `npm run build` must succeed. A broken build is the team's highest priority. Never commit on top of a broken build. *(Source: 2, 7)*
...
- **Production migrations in pre-deploy step only**: DB schema changes in staging DB during development; production DB touched only in Phase 8 step 7, immediately before `sm-deploy`. *(Source: project HARD RULES)*
- **Environment credentials never in code**: staging credentials only in `~/.envs/staff-manager.staging.env`. Production credentials never in any local file. *(Source: project HARD RULES)*
- **Deployment must be automated** (no manual steps): `sm-deploy` is the only production promotion path. Manual `git push origin main` is blocked via hook. *(Source: 2, 3)*
...
- **Hooks over instructions for critical enforcement**: HITL enforcement via `PreToolUse` hooks (production DB block, push-to-main block) is deterministic. Instructions alone are advisory. *(Source: 6, 7)*
...
- **Smoke test on staging before production**: even a 1-line fix gets 1–3 browser verification steps on staging before `sm-deploy`. *(Source: project FL-2)*
