...
- **Worktree — DB isolation (hard rule)**: the `.env.local` copied into every worktree must come exclusively from `~/.envs/staff-manager.staging.env` (staging Supabase project `gjwkvgfwkdwzqlvudgqr`). Never copy production credentials into a worktree or any local `.env.local`. If `~/.envs/staff-manager.staging.env` does not exist, stop and instruct the user to run `smenv-save staging` from the main repo before continuing.
