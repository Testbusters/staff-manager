...
**Key surfaces**: `/corsi` (admin list), `/corsi/nuovo` (create), `/corsi/[id]` (detail tabs), `/corsi/assegnazione` (resp.citt), `/corsi/valutazioni` (resp.citt), `/import` (admin - tab Corsi), `/api/admin/import-corsi/run` (admin GSheet import POST), `/impostazioni?tab=blacklist`, `/impostazioni?tab=allegati_corsi`
**Notes**: `lezioni.ore` is a GENERATED ALWAYS AS column (computed from orario_inizio/orario_fine). `lezioni.materie` is `TEXT[] NOT NULL` with CHECK `array_length(materie, 1) >= 1` (migration 069 — renamed from scalar `materia`, supports multi-materia e.g. `['Matematica', 'Fisica']`). `corsi.codice_identificativo` is UNIQUE (migration 069). `candidature` requires exactly one of `lezione_id` or `corso_id`; for `docente_lezione` candidature, API enforces overlap between `collaborator.materie_insegnate` and `lezione.materie` (422 if no overlap). `blacklist` enforces UNIQUE on `collaborator_id`. `allegati_globali` enforces UNIQUE on `(tipo, community_id)` — use UPSERT for updates.
