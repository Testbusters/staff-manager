...
FR-CORSI-13: The Responsabile Cittadino shall be able to assign a valutazione (numeric 1–10) per collaboratore×ruolo×materia on the /corsi/valutazioni page. For docente: valutazione is scoped per materia (stored on assegnazioni matching collaborator + ruolo + lezione.materia); input is locked until the docente covers ≥80% of lezioni for that materia. For cocoda: a single valutazione applies to all lezioni (no materia split), always unlocked. The UI groups entries by ruolo section (CoCoDa first, then Docente grouped by materia). An E18 email and Telegram notification are sent fire-and-forget when a valutazione is saved.
...
FR-CORSI-25: The Responsabile Cittadino shall be able to assign CoCoD'à and Q&A roles at corso level (applying to all lezioni in the corso) via dedicated panels in /corsi/assegnazione. POST /api/assegnazioni/corso accepts { corso_id, collaborator_ids, ruolo: 'cocoda'|'qa' }. Constraints: max 2 CoCoD'à per corso; max max_qa_per_lezione Q&A per corso (from corsi table). Duplicate assegnazioni are skipped (idempotent). E13 email sent fire-and-forget per collaboratore with the appropriate role label.
