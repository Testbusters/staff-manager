...
FR-REIMB-10: RLS policies on reimbursement tables shall enforce the RBAC matrix at the database level as defense-in-depth. Collaboratori may UPDATE only their own IN_ATTESA records (with WITH CHECK). Responsabile Compensi has SELECT-only access scoped to their communities. The Responsabile Compensi UPDATE policy is permanently removed. Attachment INSERT is restricted to collaboratore-owned IN_ATTESA expenses.
...
FR-USER-09: The collaboratore profile shall capture integrative data in addition to core anagrafica: (a) documento identità — numero, tipo (Carta d'Identità / Passaporto / Patente), scadenza; (b) alimentazione — regime_alimentare (onnivoro / vegetariano / vegano, default onnivoro) and an optional flag `ha_allergie_alimentari` with a descriptive note `allergie_note`; (c) indirizzo di spedizione — by default set to the residence address (`spedizione_usa_residenza=true`); when set to false the full address (indirizzo, civico, CAP, città, provincia) is mandatory, enforced by DB CHECK constraint and Zod `superRefine`. Integrative data is collected for the first time in the onboarding wizard (Step 3 — Integrativi) and may be edited thereafter on the /profilo page via three Collapsible sections (Documento identità, Alimentazione, Spedizione). The Amministrazione may prefill the documento identità section at invite time via the CreateUserForm; the Responsabile Compensi does not see integrative fields (conservative RBAC — not exposed in CollaboratoreDetail UI).
FR-USER-10: When `ha_allergie_alimentari` transitions from false to true, the system shall require the collaboratore to grant explicit GDPR Art.9 consent to the processing of health-related data via a dedicated checkbox shown alongside a prominent Alert. The timestamp of the consent is persisted server-side on `user_profiles.data_consenso_dati_salute`. When the flag transitions back to false, the timestamp is cleared to NULL (consent revoked). The `data_consenso_dati_salute` column is never editable from the client; it is derived server-side in every entry point that touches the allergie flag (onboarding, /api/profile self-edit, /api/admin/collaboratori/[id]/profile).
FR-USER-11: Existing collaboratori who completed onboarding before the introduction of integrative fields shall not be forced to re-complete onboarding. The system shall surface a soft Sonner toast on each login when `onboarding_completed=true` AND `tipo_documento_identita IS NULL`, inviting them to complete the missing information from the /profilo page. The toast is dismissible; after 3 dismissals it is no longer shown (persisted in localStorage).
...
## Part VII — Changelog
2026-04-19  |  v2.18  |  Block profilo-dati-integrativi: added FR-USER-09 (integrative profile fields — documento identità 3 cols, alimentazione 3 cols, spedizione 7 cols; collected at onboarding Step 3, edited thereafter via 3 Collapsible sections on /profilo; admin prefill via CreateUserForm; responsabile excluded from UI exposure — R7 conservative RBAC), FR-USER-10 (GDPR Art.9 consent flow for allergie data — explicit consent checkbox with Alert, server-derived `data_consenso_dati_salute` timestamp on user_profiles, revoked when flag cleared), FR-USER-11 (soft backfill Sonner toast for existing collaboratori with missing documento identità — dismissible 3x via localStorage). Migration 078: 13 cols on collaborators + 1 col on user_profiles + conditional CHECK on spedizione fields. OnboardingWizard restructured 2→4 steps with progress 25/50/75/100. Resolves R-INLINE-DRIFT: profileFormSchema/onboardingSchema extracted from inline z.object definitions in /api/profile + /api/onboarding/complete and imported from lib/schemas/collaborator.ts. Zod superRefine cross-field validation mirrors DB CHECK. 4 API routes updated (/api/profile, /api/onboarding/complete, /api/admin/create-user, /api/admin/collaboratori/[id]/profile). Admin collaboratori/[id] SELECT extended 23→36 cols.
...
2026-04-17  |  v2.1  |  Block expense-rls-hardening: added FR-REIMB-10 (RLS defense-in-depth — migration 077 fixes 3 dead policies referencing removed INVIATO/INTEGRAZIONI_RICHIESTE states). Security hardening: responsabile_compensi removed from all expense approve/reject/liquidate code paths (transitions lib + API routes). Bug fixes: floating promise on approve button (try/catch/finally), phantom INVIATO in rate-limit query.
