...
> Last updated 2026-04-16. Block g6-performance ✅ (2026-04-16).
...
| 2026-04-16 | g6-performance — G6 Performance: parallelize dashboard queries, batch DB writes, CLS fix | ✅ | tsc ✅, build ✅, vitest 478/478 ✅ | No migration. Scope: 5 MEDIUM backlog items (PERF-3, PERF-4, PERF-6, DB11, DB12). PERF-3: `adminCollab` query moved into existing 25-query `Promise.all` (admin dashboard — saves ~50-100ms sequential fetch). PERF-4: `collaborator_communities` query moved into main 10-query `Promise.all` (collab dashboard — saves 1 sequential round-trip). PERF-6: added `width`/`height` to 4 raw `<img>` tags (avatars 56x56, resp.citt avatar 48x48, discount logo 120x48); SignaturePad skipped (below-fold). DB11: sequential `for` loop YTD UPDATE replaced with `Promise.all` in both `bulk-approve` routes (compensations + expenses). DB12: N+1 notification INSERT loop replaced with single batch `.insert(array)` in `liquidazione-requests`. 6 files changed (+55 -48). Excluded from scope: PERF-5 (architectural), PERF-7 (20+ routes blast radius), P3 (premature optimization), P4 (requires migration). Phase 3b/4/4b/5b/5c ⏸ N/A (internal perf, no UI/API contract change). **Production deploy DEFERRED** per user directive. |
| 2026-04-16 | g5-dry-quality — G5 DRY / Code Quality: centralize duplicated constants, server-only guard, interface cleanup | ✅ | tsc ✅, build ✅, vitest 478/478 ✅ | No migration. Scope: 8 MEDIUM backlog items (DEV-1, DEV-2, DEV-8, DEV-12, DEV-13, T6, A1, A2). DEV-1: `NOTIFICATION_TYPE_BADGE` centralized in `lib/notification-utils.ts` with dark-mode-safe semantic classes — removed duplicate maps from NotificationBell.tsx + NotificationPageClient.tsx. DEV-2: `TSHIRT_SIZES` as `const` array + `TshirtSize` type in `lib/types.ts` — replaced 3 inline arrays in ProfileForm, OnboardingWizard (was missing XXXL — **bug fix**), CollaboratoreDetail + Zod `z.enum(TSHIRT_SIZES)` in 2 API routes. DEV-8: `import 'server-only'` on `lib/email-template-service.ts`; client-safe pure functions (`buildPreviewHtml`, `substituteMarkers`, types) extracted to new `lib/email-preview-utils.ts`; EmailTemplateManager + EmailLayoutEditor updated to import from preview utils. DEV-12: invite tracking badges in CollaboratoreDetail — hardcoded colors → semantic dark-mode tokens, `text-[10px]` → `text-xs`. DEV-13: `InviteTrackingProps` interface extracted, `CollaboratoreDetailProps extends InviteTrackingProps`. T6: `UserProfile` interface extended with `is_active`, `member_status`, `onboarding_completed`, `invite_email_sent`, `must_change_password`. A1: `lib/notification-service.ts` re-export facade (all builders + helpers). A2: `console.error` added to `getRenderedEmail` catch path. 14 files changed (+232 -106). Phase 3b/4/4b/5b/5c ⏸ N/A (internal code quality, no UI/API contract change). **Production deploy DEFERRED** per user directive. |
| 2026-04-16 | g4-api-design — G4 API Design: Zod error shape, Zod on 13 content/ticket routes, TransitionResult reason_code, flagErr logging, file size pre-validation | ✅ | tsc ✅, build ✅, vitest 478/478 ✅ | No migration. Scope: 7 backlog items (API9, API6, API3, API2, API12, API-4, S7). API9: standardized Zod error shape `{ error: 'Dati non validi', issues }` across 6 occurrences in 5 corsi/blacklist routes. API6: added Zod validation to 13 content/ticket write routes (tickets, opportunities, communications, resources, events, discounts — POST+PATCH each). API3: already resolved (all 5 state-conflict routes already used 409). API2: enriched `TransitionResult` with `reason_code: 'role'\|'state'\|'note'\|'unknown'` in both compensation-transitions.ts and expense-transitions.ts; transition routes now return 403 for role failures, 409 for state conflicts. API12: `POST /api/auth/change-password` now logs `flagErr` via `console.error` and returns `warning: 'Flag not cleared'` in response. API-4: already resolved (field already named `errorCount`). S7: added `file.size` pre-validation before `Buffer.from()` on 6 upload routes (documents POST 10MB, documents PATCH 10MB, cu-batch ZIP 50MB + CSV 2MB, contract-templates 10MB, allegati-corsi 10MB, ticket attachments 10MB) with 413 status. 28 files changed (+276 -159). Phase 3b/4/4b/5b/5c ⏸ N/A (API-internal hardening, no UI/contract change). **Production deploy DEFERRED** per user directive. |
