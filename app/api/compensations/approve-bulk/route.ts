...
export async function POST(request: Request) {
  ...
  // Fetch the requested compensations
  ...
  // Responsabile: verify community access for each compensation
  if (role === 'responsabile_compensi') {
    ...
    const unauthorized = (comps ?? []).filter((c) => !managedIds.has(c.community_id));

    if (unauthorized.length > 0) {
      return NextResponse.json(
        { error: 'Alcuni compensi non appartengono alle community a te assegnate.' },
        { status: 403 },
      );
    }
  }
