...
export async function POST(request: Request) {
  ...
  // Fetch compensations with full details for massimale check
  ...
  const collabIds = [...new Set((comps ?? []).map((c) => c.collaborator_id))];
  ...
  const byCollab = new Map<string, Array<{ id: string; collaborator_id: string; importo_lordo: number | null; data_competenza: string | null; nome_servizio_ruolo: string | null }>>();
  for (const comp of (comps ?? [])) {
    if (!byCollab.has(comp.collaborator_id)) byCollab.set(comp.collaborator_id, []);
    byCollab.get(comp.collaborator_id)!.push(comp);
  }
  ...
  for (const [collabId, items] of byCollab) {
    ...
    const totale = items.reduce((s, c) => s + (c.importo_lordo ?? 0), 0);
    ...
    if (collab && isOverMassimale(getYtd(collab), totale, collab.importo_lordo_massimale)) {
      ...
      blocked.push({
        ...
        items: items.map((c) => ({
          ...
          label: c.nome_servizio_ruolo ?? null,
          date: c.data_competenza ?? null,
        })),
      });
    } else {
      allowedIds.push(...items.map((c) => c.id));
    }
  }
  ...
  if (allowedIds.length > 0) {
    ...
    for (const comp of (comps ?? [])) {
      if (allowedIds.includes(comp.id)) {
        deltaByCollab.set(
          comp.collaborator_id,
          (deltaByCollab.get(comp.collaborator_id) ?? 0) + (comp.importo_lordo ?? 0),
        );
      }
    }
