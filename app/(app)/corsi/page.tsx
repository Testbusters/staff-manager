...
export default async function CorsiPage({
  ...
  if (role === 'collaboratore') {
    ...
    const [{ data: lezioniRaw }] = await Promise.all([
      lezioneIds.length > 0
        ? svc.from('lezioni').select('id, corso_id, data, orario_inizio, orario_fine, ore').in('id', lezioneIds)
        : Promise.resolve({ data: [] as { id: string; corso_id: string; data: string; orario_inizio: string; orario_fine: string; ore: number }[] }),
    ]);
