    ...
    <div className="flex gap-0 min-h-[600px]">
      ...
      <div className="w-[220px] shrink-0 border-r border-border overflow-y-auto">
        ...
        {GROUP_ORDER.map((group) => {
          ...
          return (
            <div key={group}>
              <div className="px-4 py-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider bg-muted/40">
                {GROUP_LABELS[group]}
              </div>
              {items.map((tmpl) => (
                <div
                ...
                >
                  <span className={`text-[11px] font-mono shrink-0 mt-0.5 ${currentKey === tmpl.key ? 'text-white/70' : 'text-muted-foreground'}`}>
                    {tmpl.key}
                  </span>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {editingTemplate.available_markers.map((marker) => (
                    <button
                      ...
                      className="flex items-center gap-1 rounded bg-muted px-2 py-0.5 text-[11px] font-mono text-muted-foreground hover:bg-muted/80 transition cursor-pointer"
                    >
                      <code>{`{{${marker}}}`}</code>
                      <Copy size={10} />
                    </button>
      ...
      {isTemplate && previewHtml && (
        <div className="w-[360px] shrink-0 border-l border-border flex flex-col">
          ...
          <div className="px-4 py-2 border-t border-border">
            <p className="text-[10px] text-muted-foreground">Anteprima aggiornata al salvataggio</p>
          </div>
        </div>
      )}
    </div>
