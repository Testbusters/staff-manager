...
type Tab = 'compensi' | 'rimborsi';

export default function CompenseTabs({ compensations, expenses, role }: Props) {
  const [activeTab, setActiveTab] = useState<Tab>('compensi');
      ...
      <div className="flex items-center gap-1 mb-5">
        ...
        >
          ...
          <span className={`rounded-full px-1.5 py-0.5 text-xs font-semibold ${
            activeTab === 'compensi' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-400'
          }`}>
        ...
        >
          ...
          Rimborsi
          <span className={`rounded-full px-1.5 py-0.5 text-xs font-semibold ${
            activeTab === 'rimborsi' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-400'
          }`}>
            {expenses.length}
          </span>
        </button>
